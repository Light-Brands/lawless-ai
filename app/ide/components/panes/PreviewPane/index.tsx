...
export function PreviewPane() {
  ...
  // Mobile: Handle double-tap to refresh
  ...
  // Mobile: Navigate back in iframe
  ...
  // Mobile: Navigate forward in iframe
  ...
  // Mobile: Open preview in new browser tab
  const handleOpenExternal = useCallback(() => {
    haptic('medium');
    const url = previewMode === 'local'
      ? getLocalPreviewUrl()
      : (latestDeployment ? `https://${latestDeployment.url}` : null);
    if (url) {
      window.open(url, '_blank');
    }
  }, [previewMode, getLocalPreviewUrl, latestDeployment]);
  ...
  return (
    <div className="preview-pane">
      ...
      {/* Preview iframe */}
      {isMobile ? (
        <PullToRefresh onRefresh={handleRefresh} className="preview-content" onClick={handleDoubleTap}>
          {isInIframe ? (
      ...
      {/* Mobile Action Bar */}
      {isMobile && (
        <div className="preview-mobile-action-bar">
          <button
          ...
          <button
            className="preview-action-btn"
            onClick={() => { haptic('light'); setShowPreviewSettings(!showPreviewSettings); }}
            title="Settings"
          >
      ...
      {/* Preview Settings Modal (Mobile) */}
      {isMobile && showPreviewSettings && (
        <>
          <div className="preview-settings-backdrop" onClick={() => setShowPreviewSettings(false)} />
          <div className="preview-settings-modal">
            <div className="preview-settings-header">
              <span>Preview Settings</span>
              <button onClick={() => setShowPreviewSettings(false)}>✕</button>
            </div>
            <div className="preview-settings-content">
